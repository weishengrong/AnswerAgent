...
面向业务数据库的自然语言问答（Text-to-SQL）后端：**FastAPI** + **Pydantic AI** 工作流，集成混合 RAG（向量 + BM25）、分层记忆（Redis / ChromaDB）、SQL 校验与安全执行，并对复杂查询提供 **ReAct** 多步推理与结构化错误反思闭环。
...
- **RAG**：ChromaDB 子块向量检索 + Redis 父块拼装；BM25（jieba）与向量结果 **RRF 融合**；可选从 DDL 重建索引。
- **记忆**：Redis 会话滑动窗口与 Token 阈值摘要；长期记忆经 **mem0** 写入 ChromaDB，入库前 **MemoryFilter** 多维度加权过滤。
