...
class ChunkBuilder:
    def __init__(self, knowledge: List[Dict] = None):
        self.tables: Dict[str, TableInfo] = {}
        self._knowledge = knowledge or []
        self._field_knowledge_map: Dict[str, List[str]] = {}

    def _build_field_knowledge_map(self):
        for entry in self._knowledge:
            knowledge_text = (
                f"{entry.get('knowledge', '')} "
                f"{entry.get('description', '')} "
                f"{entry.get('definition', '')}"
            ).lower()
            for table_name, table in self.tables.items():
                for field in table.fields:
                    if field.name.lower() in knowledge_text:
                        key = f"{table_name}.{field.name}".lower()
                        if key not in self._field_knowledge_map:
                            self._field_knowledge_map[key] = []
                        self._field_knowledge_map[key].append(
                            f"{entry.get('knowledge', '')} - {entry.get('description', '')}"
                        )
    ...
    def build(self, tables: List[TableInfo]) -> Tuple[List[ParentChunk], List[ChildChunk]]:
        ...
        self._build_field_knowledge_map()
    ...
    def _build_embedding_content(
        ...
        raw_desc = field.desc if field.desc else ""
        desc = str(raw_desc).strip() if not isinstance(raw_desc, str) else raw_desc.strip()

        if not desc:
            content = f"表{table_name}的{field.name}字段"
        else:
            content = f"表{table_name}的{field.name}字段：{desc}"

        key = f"{table_name}.{field.name}".lower()
        if key in self._field_knowledge_map:
            for knowledge_str in self._field_knowledge_map[key]:
                content += f"\n关联知识：{knowledge_str}"
...
def build_chunks(tables: List[TableInfo], knowledge: List[Dict] = None) -> Tuple[List[ParentChunk], List[ChildChunk], Dict[str, List[Dict]]]:
    builder = ChunkBuilder(knowledge=knowledge)
